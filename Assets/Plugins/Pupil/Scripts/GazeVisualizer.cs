...
namespace PupilLabs
{
    public class GazeVisualizer : MonoBehaviour
    {
        ...
        public bool movingScotoma = true;
        ...
        void ShowProjected()
        {
            ...
            if (Physics.SphereCast(origin, sphereCastRadius, direction, out RaycastHit hit, maxDistance))
            {
                ...
                gazeDirectionMarker.position = origin + direction * maxDistance;
                gazeDirectionMarker.LookAt(origin);

                if (errorAngleBasedMarkerRadius)
                {
                    gazeDirectionMarker.localScale = GetErrorAngleBasedScale(origMarkerScale, maxDistance, angleErrorEstimate);
                }
            }
            else
            {
